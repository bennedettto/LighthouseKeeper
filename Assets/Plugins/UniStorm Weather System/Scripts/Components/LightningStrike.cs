    ...
    public class LightningStrike : MonoBehaviour
    {
        ...
        void OnTriggerEnter(Collider C)
        {
            if (C.gameObject.layer != 2 && C.GetComponent<Terrain>() == null && (DetectionLayerMask.value & 1 << C.gameObject.layer) != 0)
            {
                ...
                else if (!C.CompareTag(PlayerTag) && !C.CompareTag(EmeraldAITag) && UniStormSystem.Instance.LightningStrikesEmeraldAI == UniStormSystem.EnableFeature.Enabled ||
                    UniStormSystem.Instance.LightningStrikesEmeraldAI == UniStormSystem.EnableFeature.Disabled && !C.CompareTag(PlayerTag))
                {
                    ObjectDetected = true;
                    HitPosition = C.transform.position;
                    HitObject = C.gameObject;
                }
                else if (C.CompareTag(EmeraldAITag) && UniStormSystem.Instance.LightningStrikesEmeraldAI == UniStormSystem.EnableFeature.Enabled)
                {
