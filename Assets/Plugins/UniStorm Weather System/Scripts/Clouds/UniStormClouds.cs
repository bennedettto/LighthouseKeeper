...
namespace UniStorm.Utility
{
    public class UniStormClouds : MonoBehaviour
    {
        ...
        void GenerateInitialNoise()
        {
            ...
            skyMaterial.SetFloat(uLightningTimer, 0.0f);
        ...
        static readonly int uLightningTimer = Shader.PropertyToID("_uLightningTimer");
        static readonly int uBaseNoise = Shader.PropertyToID("_uBaseNoise");
        static readonly int uDetailNoise = Shader.PropertyToID("_uDetailNoise");
        static readonly int uCurlNoise = Shader.PropertyToID("_uCurlNoise");
        static readonly int uCloudsMovementSpeed = Shader.PropertyToID("_uCloudsMovementSpeed");
        static readonly int uCloudsTurbulenceSpeed = Shader.PropertyToID("_uCloudsTurbulenceSpeed");
        static readonly int uBaseCloudOffset = Shader.PropertyToID("_uBaseCloudOffset");
        static readonly int uDetailCloudOffset = Shader.PropertyToID("_uDetailCloudOffset");
        static readonly int uSize = Shader.PropertyToID("_uSize");
        static readonly int uCount = Shader.PropertyToID("_uCount");
        static readonly int uJitter = Shader.PropertyToID("_uJitter");
        static readonly int uRaymarchOffset = Shader.PropertyToID("_uRaymarchOffset");
        static readonly int uSunDir = Shader.PropertyToID("_uSunDir");
        static readonly int uMoonDir = Shader.PropertyToID("_uMoonDir");
        static readonly int uWorldSpaceCameraPos = Shader.PropertyToID("_uWorldSpaceCameraPos");
        ...
        void CloudsUpdate()
        {
            ...
            skyMaterial.SetTexture(uBaseNoise, GenerateNoise.baseNoiseTexture);
            skyMaterial.SetTexture(uDetailNoise, GenerateNoise.detailNoiseTexture);
            skyMaterial.SetTexture(uCurlNoise, GenerateNoise.curlNoiseTexture);

            baseCloudOffset += skyMaterial.GetFloat(uCloudsMovementSpeed) * Time.deltaTime;
            detailCloudOffset += skyMaterial.GetFloat(uCloudsTurbulenceSpeed) * Time.deltaTime;

            skyMaterial.SetFloat(uBaseCloudOffset, baseCloudOffset);
            skyMaterial.SetFloat(uDetailCloudOffset, detailCloudOffset);

            skyMaterial.SetFloat(uSize, size);
            skyMaterial.SetInt(uCount, frameCount);
            skyMaterial.SetVector(uJitter, new Vector2(offsetX, offsetY));
            skyMaterial.SetFloat(uRaymarchOffset, (haltonSequence[haltonSequenceIndex] / 16.0f + bayerOffsets[offset[frameIndex, 0], offset[frameIndex, 1]] / 16.0f));

            skyMaterial.SetVector(uSunDir, sun.transform.forward);
            skyMaterial.SetVector(uMoonDir, Vector3.Normalize(moon.forward));
            skyMaterial.SetVector(uWorldSpaceCameraPos, UniStormSystem.Instance.PlayerCamera.transform.position);

            //Update the cloudsMaterial with the latest texture
            cloudsMaterial.SetTexture(mainTex, fullCloudsBuffer[fullBufferIndex ^ 1]);
        }
