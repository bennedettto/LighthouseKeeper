...
namespace UniStorm.Effects
{
    ...
    public class UniStormSunShafts : UniStormPostEffectsBase
    {
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination) {
            ...
            sunShaftsMaterial.SetVector (ShaderConstants.blurRadius4, new Vector4 (1.0f, 1.0f, 0.0f, 0.0f) * sunShaftBlurRadius );
            sunShaftsMaterial.SetVector (ShaderConstants.sunPosition, new Vector4 (v.x, v.y, v.z, maxRadius));
            sunShaftsMaterial.SetVector (ShaderConstants.threshold, sunThreshold);
            ...
            if (!useDepthTexture) {
                ...
                sunShaftsMaterial.SetTexture (ShaderConstants.skybox, tmpBuffer);
            ...
            sunShaftsMaterial.SetVector(ShaderConstants.blurRadius4, new Vector4 (ofs, ofs, 0.0f, 0.0f));
            sunShaftsMaterial.SetVector (ShaderConstants.sunPosition, new Vector4 (v.x, v.y, v.z, maxRadius));
            ...
            if (v.z >= 0.0f)
                sunShaftsMaterial.SetVector (ShaderConstants.color, new Vector4 (sunColor.r, sunColor.g, sunColor.b, sunColor.a) * sunShaftIntensity);
            else
                sunShaftsMaterial.SetVector (ShaderConstants.color, Vector4.zero); // no backprojection !
            sunShaftsMaterial.SetTexture (ShaderConstants.colorBuffer, lrDepthBuffer);
