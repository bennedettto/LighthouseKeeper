...
public class UniStormAtmosphericFogPass : ScriptableRenderPass
{
    ...
    private void SetupDirectionalLights()
    {
        ...
        foreach (Light light in lights)
        {
            ...
            if (light.gameObject.name.IndexOf("sun", StringComparison.OrdinalIgnoreCase) >= 0)
                settings.SunSource = light;
            if (light.gameObject.name.IndexOf("moon", StringComparison.OrdinalIgnoreCase) >= 0)
                settings.MoonSource = light;
        }
    }
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        fogMaterial.SetMatrixArray(ShaderConstants.frustumCornersWsArray, new Matrix4x4[2] { leftEyeFrustumMatrix, rightEyeFrustumMatrix });

        fogMaterial.SetVector(ShaderConstants.cameraWs, camPos);
        fogMaterial.SetVector(ShaderConstants.heightParams, new Vector4(settings.height, FdotC, paramK, settings.heightDensity * 0.5f));
        fogMaterial.SetVector(ShaderConstants.distanceParams, new Vector4(-Mathf.Max(settings.startDistance, 0.0f), 0, 0, 0));

        if (settings.SunSource != null)
            fogMaterial.SetVector(ShaderConstants.sunVector, settings.SunSource.transform.rotation * -Vector3.forward);
        if (settings.MoonSource != null)
            fogMaterial.SetVector(ShaderConstants.moonVector, settings.MoonSource.transform.rotation * -Vector3.forward);
        fogMaterial.SetFloat(ShaderConstants.sunIntensity, settings.SunIntensity);
        fogMaterial.SetFloat(ShaderConstants.moonIntensity, settings.MoonIntensity);
        fogMaterial.SetFloat(ShaderConstants.sunAlpha, settings.SunFalloffIntensity);
        fogMaterial.SetColor(ShaderConstants.sunColor, settings.SunColor);
        fogMaterial.SetColor(ShaderConstants.moonColor, settings.MoonColor);

        fogMaterial.SetColor(ShaderConstants.upperColor, settings.TopColor);
        fogMaterial.SetColor(ShaderConstants.bottomColor, settings.BottomColor);
        fogMaterial.SetFloat(ShaderConstants.fogBlendHeight, settings.BlendHeight);
        fogMaterial.SetFloat(ShaderConstants.fogGradientHeight, settings.FogGradientHeight);

        fogMaterial.SetFloat(ShaderConstants.sunControl, settings.SunControl);
        fogMaterial.SetFloat(ShaderConstants.moonControl, settings.MoonControl);
        ...
        if (settings.Dither == UniStormAtmosphericFogFeature.DitheringControl.Enabled)
        {
            ...
            fogMaterial.SetTexture(ShaderConstants.noiseTex, settings.NoiseTexture);
        }
        else
        {
        ...
        fogMaterial.SetVector(ShaderConstants.sceneFogMode, new Vector4((int)sceneMode, settings.useRadialDistance ? 1 : 0, 0, 0));
